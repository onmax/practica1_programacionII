...
public class VentanillaVirtualUsuario {
	...
	private final String[] opcionesMenuPrincipal = { "Comprar Entrada", "Recoger Entrada", "Consultar Estado Sesión",
			"Comprar con recomendación de butacas", "Salir" };
	private final String[] opcionesObligatoriasMenuPrincipal = { "Comprar Entrada", "Consultar Estado Sesión",
			"Salir" };
	...
	/**
	 * pedimos una sala y una sesion validas al usuario se guardan en los
	 * atributos sala y sesion
	 */
	...
	/**
	 * pedimos una sala, una sesion y una butaca validas al usuario se guardan
	 * en los atributos sala, sesion y (fila, columna)
	 */
	public void pedirDatosCompra() {
		...
		if (cine.getButacasDisponiblesSesion(sala, sesion) == 0)
			...
			do {
				...
				if (fila < 0 || fila > estadoSesion.length || columna < 0 || columna > estadoSesion[0].length) {
					System.out.println("La butaca seleccionada " + "no existe, elige otra");
					sigue = true;
				} else
				// comprobamos si la butaca esta ya vendida
				if (estadoSesion[fila - 1][columna - 1] == '#') {
					System.out.println("La butaca seleccionada " + "está ocupada, elige otra");
					sigue = true;
				}
	...
	/**
	 * pedimos una sala y una sesion validas al usuario, y un id de compra se
	 * guardan en los atributos id, sala y sesion
	 */
	...
	/**
	 * pedimos una sala y una sesion validas al usuario, y un no. de butacas
	 * contiguas se guardan en los atributos noButacas, sala y sesion
	 */
