...
public class TestAtributosSesion {
	...
	private static final String CLAZZ = PKG + "Sesion";
	...
	public static void setup () {
		...
		atributos.add(new Atributo("asientosDisponibles","int")); 		
		atributos.add(new Atributo("sigIdCompra","int"));
		atributos.add(new Atributo("hora","java.lang.String"));
		atributos.add(new Atributo("estadoAsientos","int[][]"));	
	...
	private static String convertirTipo (String tipo) {
	    if (tipo.endsWith("[][]")){
            return "[[" + getTipo(tipo.substring(0,tipo.length()-4));
        }
		if (tipo.endsWith("[]")){
