...
public class TestAtributosSala {
	...
	private static final String CLAZZ = PKG + "Sala";
	...
	public static void setup () {
		...
		atributos.add(new Atributo("columnas","int")); 		
		atributos.add(new Atributo("filas","int"));
		atributos.add(new Atributo("pelicula","java.lang.String"));
		atributos.add(new Atributo("sesiones","list.ArrayList"));	
	...
	private void testAtributo (Atributo atributo) {
		...
		try {
			...
			if (!atributo.getTipoConvertido().equals(field.getType().getName())) {
				fail(field.getType().getName() + "Se ha encontrado el atributo " + atributo.getNombre() +  " no coincide el tipo esperado (" + atributo.getTipo()+")"); 
			}
